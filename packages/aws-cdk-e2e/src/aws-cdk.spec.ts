...
describe('"@stellar-libs/aws-cdk" Generators', () => {
  beforeAll(() => {
    ensureNxLocalLibs();
    ensureNxProject('@stellar-libs/aws-cdk', 'dist/packages/aws-cdk');
  });
  ...
  describe('Application Generator', () => {
    it('generate application with duplicate name (error)', async () => {
      ...
      await runNxCommandAsync(
        `generate @stellar-libs/aws-cdk:application ${pluginName}`
      );
    ...
    it('generate application with custom directory', async () => {
      ...
      await runNxCommandAsync(
        `generate @stellar-libs/aws-cdk:application ${pluginName} --directory ${pluginDirectory}`
      );
      expect(() =>
    ...
    it('generate application with no jest unit test (test dir should not exist)', async () => {
      const pluginName = uniq('aws-cdk');
      await runNxCommandAsync(
        `generate @stellar-libs/aws-cdk:application ${pluginName} --jest false`
      );
      expect(() =>
    ...
    it('generate application with no linting (.eslintrc file should not exist)', async () => {
      const pluginName = uniq('aws-cdk');
      await runNxCommandAsync(
        `generate @stellar-libs/aws-cdk:application ${pluginName} --linting false`
      );
      expect(() =>
  ...
  describe('Remove Generator', () => {
    ...
    it('remove application', async () => {
      await runNxCommandAsync(
        `generate @stellar-libs/aws-cdk:remove ${pluginName}`
      );
      expect(() =>
        checkFilesExist(path.join(pluginName, 'cdk.json'))
      ).toThrow();
